...
export class Client extends ClientJS {
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        const bulkUpdate = [];
        const operationToSkip = commandsToSkip.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON(new ApplicationGuildMixin(guild, cmd.instance))));
        const operationToAdd = options?.disable?.add
            ? []
            : commandsToAdd.map(async (DCommand) => bulkUpdate.push(await DCommand.toJSON(new ApplicationGuildMixin(guild, DCommand))));
        const operationToUpdate = options?.disable?.update
            ? commandsToUpdate.map(async (cmd) => bulkUpdate.push((await cmd.command.toJSON())))
            : commandsToUpdate.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON(new ApplicationGuildMixin(guild, cmd.instance))));
    ...
    isApplicationCommandEqual(commandJson, rawData) {
        ...
        if (commandJson.type === "CHAT_INPUT" && commandJson.options) {
            commandJson.options = _.map(commandJson.options, (object) => _.omit(object, [
        ...
        if (rawData.type === "CHAT_INPUT" && rawData.options) {
            rawData.options = _.map(rawData.options, (object) => _.omit(object, [
                "descriptionLocalizations",
                "descriptionLocalized",
                "nameLocalizations",
                "nameLocalized",
        ...
        return _.isEqual(JSON.parse(JSON.stringify(_.omit(commandJson, "id", "applicationId", "guild", "guildId", "version", "descriptionLocalizations", "descriptionLocalized", "nameLocalizations", "nameLocalized"))), JSON.parse(JSON.stringify(_.omit(rawData, "descriptionLocalizations", "descriptionLocalized", "nameLocalizations", "nameLocalized"))));
    }
    /**
