...
class Client extends discord_js_1.Client {
            ...
            this.simpleCommands.map((cmd) => {
                ...
                const printOptions = (options, depth) => {
                    ...
                    options.forEach((option) => {
                        this.logger.log(`${tab}${option.name}: ${index_js_1.SimpleCommandOptionType[option.type]} (${option.classRef.name}.${option.key})`);
                    });
                };
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        await Promise.all(DCommands.map(async (DCommand) => {
            ...
            const rawData = await DCommand.toJSON(new index_js_1.ApplicationGuildMixin(guild, DCommand));
        ...
        const bulkUpdate = [];
        const operationToSkip = commandsToSkip.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON(new index_js_1.ApplicationGuildMixin(guild, cmd.instance))));
        const operationToAdd = options?.disable?.add
            ? []
            : commandsToAdd.map(async (DCommand) => bulkUpdate.push(await DCommand.toJSON(new index_js_1.ApplicationGuildMixin(guild, DCommand))));
        const operationToUpdate = options?.disable?.update
            ? commandsToUpdate.map(async (cmd) => bulkUpdate.push((await cmd.command.toJSON())))
            : commandsToUpdate.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON(new index_js_1.ApplicationGuildMixin(guild, cmd.instance))));
    ...
    isApplicationCommandEqual(commandJson, rawData) {
        ...
        if (commandJson.type === "CHAT_INPUT" && commandJson.options) {
            commandJson.options = lodash_1.default.map(commandJson.options, (object) => lodash_1.default.omit(object, [
        ...
        if (rawData.type === "CHAT_INPUT" && rawData.options) {
            rawData.options = lodash_1.default.map(rawData.options, (object) => lodash_1.default.omit(object, [
                "descriptionLocalizations",
                "descriptionLocalized",
                "nameLocalizations",
                "nameLocalized",
        ...
        return lodash_1.default.isEqual(JSON.parse(JSON.stringify(lodash_1.default.omit(commandJson, "id", "applicationId", "guild", "guildId", "version", "descriptionLocalizations", "descriptionLocalized", "nameLocalizations", "nameLocalized"))), JSON.parse(JSON.stringify(lodash_1.default.omit(rawData, "descriptionLocalizations", "descriptionLocalized", "nameLocalizations", "nameLocalized"))));
    }
    /**
    ...
    async initGlobalApplicationCommands(options) {
        ...
        // perform bulk update with discord using set operation
        const bulkUpdate = [];
        const operationToSkip = commandsToSkip.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON()));
        const operationToAdd = options?.disable?.add
            ? []
            : commandsToAdd.map(async (DCommand) => bulkUpdate.push(await DCommand.toJSON()));
        const operationToUpdate = options?.disable?.update
            ? commandsToUpdate.map(async (cmd) => bulkUpdate.push((await cmd.command.toJSON())))
            : commandsToUpdate.map(async (cmd) => bulkUpdate.push(await cmd.instance.toJSON()));
        const operationToDelete = options?.disable?.delete
            ? commandsToDelete.map(async (cmd) => bulkUpdate.push((await cmd.toJSON())))
            : [];
        await Promise.all([
    ...
    async initApplicationPermissions(log) {
        ...
        this.logger.warn("\n\n");
        this.logger.warn("************************************************************");
        this.logger.warn("Discord has deprecated permissions v1 api in favour permissions v2, await future updates");
    ...
    async executeComponent(components, interaction, log) {
        ...
        if ((log ?? !this.silent) && !results.some((res) => res)) {
            this.logger.error(`${this.user?.username ?? this.botId} >> ${interaction.isButton() ? "button" : "select menu"} component handler not found, interactionId: ${interaction.id} | customId: ${interaction.customId}`);
        }
        return results;
    }
    /**
