...
class MetadataStorage {
    ...
    addDiscord(discord) {
        ...
        di_1.DIService.instance.addService(discord.classRef);
    }
    addGuard(guard) {
        this._guards.push(guard);
        di_1.DIService.instance.addService(guard.classRef);
    }
    ...
    async build() {
        ...
        this.discordMembers.forEach((member) => {
            ...
            if (member instanceof index_js_1.DApplicationCommand) {
                discord.applicationCommands.push(member);
            }
            if (member instanceof index_js_1.DSimpleCommand) {
                discord.simpleCommands.push(member);
            }
            if (member instanceof index_js_1.DReaction) {
                discord.reactions.push(member);
            }
            if (member instanceof index_js_1.DOn) {
                discord.events.push(member);
            }
            if (member instanceof index_js_1.DComponent) {
                if (member.type === index_js_1.ComponentType.Button) {
                    discord.buttons.push(member);
                }
                else if (member.type === index_js_1.ComponentType.SelectMenu) {
                    discord.selectMenus.push(member);
                }
            }
        });
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._discords);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._events);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._applicationCommandSlashes);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._applicationCommandSlashOptions);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._applicationCommandMessages);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._applicationCommandUsers);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._simpleCommands);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._simpleCommandOptions);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._buttonComponents);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._modalComponents);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._reactions);
        await internal_1.Modifier.applyFromModifierListToList(this._modifiers, this._selectMenuComponents);
    ...
    buildSimpleCommands() {
        this._simpleCommands.forEach((cmd) => {
            ...
            if (cmd.prefix) {
                [...cmd.prefix].forEach((pfx) => {
                    ...
                    mapCmd.forEach((mapCommand) => {
                        if (lodash_1.default.findIndex(commands, { name: mapCommand.name }) !== -1) {
            ...
            if (lodash_1.default.findIndex(this._simpleCommandsByName, { name: cmd.name }) !== -1) {
                throw Error(`Duplicate simple command name: ${cmd.name}`);
            }
            this._simpleCommandsByName.push({ command: cmd, name: cmd.name });
            cmd.aliases.forEach((al) => {
                if (lodash_1.default.findIndex(this._simpleCommandsByName, { name: al }) !== -1) {
    ...
    groupSlashes() {
        ...
        this._applicationCommandSlashGroups.forEach((group) => {
            const slashParent = index_js_1.DApplicationCommand.create(group.name, "CHAT_INPUT", group.infos?.description).decorate(group.classRef, group.key, group.method);
            const discord = this._discords.find((instance) => {
        ...
        this._applicationCommandSlashSubGroups.forEach((subGroup) => {
            const option = index_js_1.DApplicationCommandOption.create(subGroup.name, undefined, undefined, subGroup.infos?.description, undefined, undefined, undefined, undefined, "SUB_COMMAND_GROUP").decorate(subGroup.classRef, subGroup.key, subGroup.method);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        return async (...params) => {
            await Promise.all(eventsToExecute.map(async (ev) => {
